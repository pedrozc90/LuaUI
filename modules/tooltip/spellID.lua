...
----------------------------------------------------------------
function Tooltips:AddSpellID()
    local UnitBuff, UnitDebuff, UnitAura, UnitGUID, GetGlyphSocketInfo = UnitBuff,
    ...
    local function addLine(tooltip, id, kind)
        ...
        for i = 1, 15 do
            ...
            if text and text:find(kind .. ":") then return end
        end
    ...
    local function attachItemTooltip(self)
        ...
        for v in gmatch(itemString, "(%d*:?)") do
            if v == ":" then
                itemSplit[#itemSplit + 1] = 0
            else
                itemSplit[#itemSplit + 1] = gsub(v, ":", "")
            end
        end
        ...
        for i = 1, 4 do
            ...
            if gemLink then
                local gemDetail = match(gemLink, "item[%-?%d:]+")
                gems[#gems + 1] = match(gemDetail, "item:(%d+):")
